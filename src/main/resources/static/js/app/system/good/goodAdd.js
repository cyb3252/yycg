var validator;
var $goodAddForm = $("#good-add-form");

$(function () {
    validateRule();
    $("#good-add .btn-save").click(function () {
        var name = $(this).attr("name");
        var validator = $goodAddForm.validate();
        var flag = validator.form();
        if (flag) {
            if (name === "save") {
                $.post(ctx + "good/add", $goodAddForm.serialize(), function (r) {
                    if (r.code === 0) {
                        closeModal();
                        $MB.n_success(r.msg);
            ...
            if (name === "update") {
                $.post(ctx + "good/update", $goodAddForm.serialize(), function (r) {
                    if (r.code === 0) {
                        closeModal();
                        $MB.n_success(r.msg);
                        $MB.refreshTable("goodTable");
...
function closeModal() {
    ...
    validator.resetForm();
    $("#good-add-modal-title").html('新增药品');
...
function validateRule() {
    ...
    validator = $goodAddForm.validate({
        rules: {
            goodsName: {
                required: true
                /*minlength: 3,
